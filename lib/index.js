(() => {
	...
	const registerGrammars = () => {
		jQuery.registerPegeditorGrammar("ecolang", EcoLang.parse.bind(EcoLang));
	}
	...
	const createEditors = () => {
		jQuery("#e1").pegeditor({
			...
			onSuccess: (output) => {
				if(editor2) {
					...
					localStorage.__ECOLANG_APP__ = editor1.instance.getValue();
		...
		jQuery("#e2").pegeditor({
			...
			onInitialized: (editor) => {
				console.log("initialized", editor);
				editor2 = editor;
				if(localStorage.__ECOLANG_APP__) {
					const input = localStorage.__ECOLANG_APP__;
